...
class TerrainOverviewMixin:
    ...
    def _get_terrain_scene_backend(self):
        if getattr(self, 'terrain_scene_backend', None) is None:
            requested = getattr(self, 'terrain_scene_backend_requested', 'auto')
            self.terrain_scene_backend = create_terrain_scene_backend(requested)
