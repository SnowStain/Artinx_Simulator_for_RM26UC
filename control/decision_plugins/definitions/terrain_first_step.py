...
def _pick_step_targets(controller, map_manager, team, facility_type, near_offset_m, far_offset_m):
	...
	primary = controller._resolve_navigation_target(
		(near_edge - direction * controller._meters_to_world_units(near_offset_m, map_manager), center_y),
		map_manager,
	)
	secondary = controller._resolve_navigation_target(
		(far_edge + direction * controller._meters_to_world_units(far_offset_m, map_manager), center_y),
		map_manager,
	)
	targets = {}
...
def _effective_targets(controller, role_key, decision_id, map_manager, team):
	return controller.get_decision_point_targets(role_key, decision_id, map_manager, team=team) or _pick_step_targets(controller, map_manager, team, 'first_step', 0.45, 0.78)


def terrain_condition(controller, context, role_key, binding):
	return bool(controller._best_terrain_traversal_plan(context, required_type='first_step'))
...
def terrain_action(controller, context, role_key, binding):
	...
	speed = controller._meters_to_world_units(1.9, context.map_manager)
...
def terrain_preview_points(controller, role_key, map_manager, team=None, override=None, binding=None):
	return _pick_step_targets(controller, map_manager, team or 'red', 'first_step', 0.45, 0.78)


def terrain_preview_regions(controller, role_key, map_manager, team=None, override=None, binding=None):
	decision_id = binding.get('id') if isinstance(binding, dict) else 'terrain_first_step'
	targets = _effective_targets(controller, role_key, decision_id, map_manager, team or 'red')
	radius = controller._meters_to_world_units(0.48, map_manager) if map_manager is not None else 22.0
	return [{'shape': 'circle', 'cx': float(point[0]), 'cy': float(point[1]), 'radius': float(radius)} for point in targets.values()]


PLUGIN = deepcopy(PLUGINS['terrain_first_step'])
